...
(defn gemma-block
  ...
  (let [{:keys [input-ln-w q-w k-w v-w o-w
        ...
        attn-raw (gemma-attention x-norm1 q-w k-w v-w o-w num-heads num-kv-heads pos-ids)
...
(defn full-gemma-forward
  ...
  ([x embed-tokens layers-weights final-norm-w pos-ids]
   (let [first-layer (first layers-weights)
     ...
     (full-gemma-forward x embed-tokens layers-weights final-norm-w pos-ids num-heads num-kv-heads)))
